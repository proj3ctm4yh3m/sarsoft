...
<c:forEach var="mapSource" items="${mapSources}" varStatus="status">
	<c:if test="${not(mapSource.alphaOverlay)}"><c:choose><c:when test="${first eq true}"><c:set var="first" value="${false}"/></c:when><c:otherwise>,</c:otherwise></c:choose>{name : "${mapSource.name}", alias: "${mapSource.alias}", type: "${mapSource.type}", copyright: "${mapSource.copyright}", minresolution: ${mapSource.minresolution}, maxresolution: ${mapSource.maxresolution}, png: ${mapSource.png}, alphaOverlay: ${mapSource.alphaOverlay}, info: "${mapSource.info}", template: <c:choose><c:when test="${mapSource.type eq tile and tileCacheEnabled eq true and fn:startsWith(mapSource.template, 'http')}">"/resource/imagery/tilecache/${mapSource.name}/{Z}/{X}/{Y}.png"</c:when><c:otherwise>"${mapSource.template}"</c:otherwise></c:choose>}</c:if>
</c:forEach>
<c:forEach var="mapSource" items="${mapSources}" varStatus="status">
	<c:if test="${mapSource.alphaOverlay}">,{name : "${mapSource.name}", alias: "${mapSource.alias}", type: "${mapSource.type}", copyright: "${mapSource.copyright}", minresolution: ${mapSource.minresolution}, maxresolution: ${mapSource.maxresolution}, png: ${mapSource.png}, alphaOverlay: ${mapSource.alphaOverlay}, info: "${mapSource.info}", template: <c:choose><c:when test="${mapSource.type eq tile and tileCacheEnabled eq true and fn:startsWith(mapSource.template, 'http')}">"/resource/imagery/tilecache/${mapSource.name}/{Z}/{X}/{Y}.png"</c:when><c:otherwise>"${mapSource.template}"</c:otherwise></c:choose>}</c:if>
</c:forEach>
];

org.sarsoft.EnhancedGMap.geoRefImages = [
<c:forEach var="image" items="${geoRefImages}" varStatus="status">
<c:if test="${status.index gt 0}">,</c:if>{name : "${image.name}", alias : "${image.alias}", id: ${image.id}, angle: ${image.angle}, scale : ${image.scale}, originx: ${image.originx}, originy: ${image.originy}, originlat: ${image.originlat}, originlng: ${image.originlng}, width: ${image.width}, height: ${image.height}}
</c:forEach>
];
...
<c:forEach var="source" items="${visibleSources}"><c:choose><c:when test="${first}"><c:set var="first" value="${false}"/></c:when><c:otherwise>,</c:otherwise></c:choose>"${source}"</c:forEach>
];
