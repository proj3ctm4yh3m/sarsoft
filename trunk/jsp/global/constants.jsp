...
<c:forEach var="mapSource" items="${mapSources}" varStatus="status">
	<c:if test="${status.index gt 0}">,</c:if>{name : "${mapSource.name}", type: "${mapSource.type}", copyright: "${mapSource.copyright}", minresolution: ${mapSource.minresolution}, maxresolution: ${mapSource.maxresolution}, png: ${mapSource.png}, template: <c:choose><c:when test="${mapSource.type eq tile and tileCacheEnabled eq true}">"/resource/imagery/tilecache/${mapSource.name}/{Z}/{X}/{Y}"</c:when><c:otherwise>"${mapSource.template}"</c:otherwise></c:choose>}
</c:forEach>
];
