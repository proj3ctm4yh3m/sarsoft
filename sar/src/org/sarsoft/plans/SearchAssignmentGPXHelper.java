...
//			if(clue.getAssignmentId() != null) attrs.put("assignmentid", clue.getAssignmentId());
	...
	public static void updateSearch(JSONObject json, GenericHibernateDAO dao) {
		...
		for(String id : periodAttrs.keySet()) {
			OperationalPeriod period = dao.load(OperationalPeriod.class, Long.parseLong(id));
			if(period == null) {
	...
	private static void updateAssignmentsAndWays(Map[] objs, GenericHibernateDAO dao) {
		for(Map<String, Object> obj : objs) {
			...
			OperationalPeriod period = dao.load(OperationalPeriod.class, Long.parseLong(name.substring(0, 2)));
			...
			if(period == null) {
				...
				dao.save(period);
				period = dao.load(OperationalPeriod.class, Long.parseLong(name.substring(0, 2)));
			}
